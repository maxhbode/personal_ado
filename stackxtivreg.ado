...
cap: program drop stackxtivreg

program stackxtivreg, eclass
#delimit ;
...
loc depvar1 "f_`fetype'_`dep1'" /* first stage dep */

* Second-stage dependents
foreach X in `dep2' {
	loc XX = `XX' + 1
	loc dep2_`XX' = "`X'"
	loc depvar2_`XX' = "s_`fetype'_`X'"
}
...
forval X = 1/`dep2_count' {
	...
	*loc 2ndStage_`X' = subinstr(`"Second Stage~-~`: var label `e(depvar)''"'," ","~",.)
	...
	loc newcolname "`e(instd)'_`e(depvar)'"
